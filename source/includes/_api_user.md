...
user_authenticated_clients           | array[string]        | Array of strings, each representing an [Authorized Device](#authorized-devices), on which TFA is enabled, but the device has been white-listed upon a successful TFA authentication by having set the cookie `'tfa_key'` and until it's expiration allows for Simple Authentication in place of TFA                                                                                                                                       | **&check;** |
