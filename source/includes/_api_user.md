...
**email**                            | string               | Email address of the user (must contain only ASCII characters). <br/> Also, when TFA authentication scheme is used, the e-mail messages containing TFA code will be delivered to this address. | **&check;** | **<sub><form action="#create-user"><button>PUT</button></form></sub>**
...
is_two_factor_authentication_enabled | int                  | 1 - user will be authenticated by TFA scheme, 0 - user will use simple authentication scheme | **&check;**  |
user_authenticated_clients           | list of strings | Contains the list of trusted client devices that have been used for successful authorization of this user in the past. See [Authorized devices](#authorized-devices)  | **&check;** |
...
sms_phone | string | Phone number to be used for SMS notifications. <br/>Also, when TFA authentication scheme is used, the text messages (SMS) containing TFA code will be delivered to this number | **&check;** |
