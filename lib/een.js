$(function () {
    var Workspace = Backbone.Router.extend({
        ...
        info_resource: function(resource) {
            ...
            if(elem.find('.info-resource').size()===0){
                ...
                infoResource.load('docs/'+resource+'/info.html', function() {
    ...
    var swaggerUi = new SwaggerUi({
        ...
        onComplete: function(swaggerApi, swaggerUi){
            ...
            _.each($('.resource .options'), function(item){
                var elem = $(item);
    ...
    function showTerms(e){
        ...
        if(terms.size()===0){
            ...
            terms.load('docs/terms.html', function() {
                if($.trim(terms.text())){
                    terms.prepend('<h2 class="info-resource-heading">Definition of Terms</h2>');
                    terms.show();
                    elem.text('Hide Definition of Terms');
                }else{
                    terms.hide();
                    elem.text('Show Definition of Terms');
                }
            });
    ...
    function initRoute(){
        if(completed && $('#resources_container').size()===1 && Backbone.history.fragment){
            ...
            try{
                ...
                $('html, body').animate({
                    scrollTop: $("#resource_"+Backbone.history.fragment.split('/')[1]).offset().top
                }, 1000);
            }catch(e){
                // do nothing
            }
